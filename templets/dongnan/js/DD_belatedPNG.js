var DD_belatedPNG = {
  ...
  createVmlStyleSheet: function () {
    ...
    if (b.styleSheet) {
      ...
      b.addRule("img." + this.ns + "_sizeFinder",
  ...
  vmlOpacity: function (b) {
    if (b.currentStyle.filter.search("lpha") != -1) {
      ...
      a = parseInt(a.substring(a.lastIndexOf("=") + 1, a.lastIndexOf(")")), 10)
          / 100;
  ...
  vmlOffsets: function (d) {
    ...
    e(d.vml.color.shape, (n.L + (d.isImg ? 0 : n.bLW)),
        (n.T + (d.isImg ? 0 : n.bTW)), (n.W - 1), (n.H - 1), 0);
    ...
    if (m != "repeat" || d.isImg) {
      ...
      d.vml.image.shape.style.clip = "rect(" + i.T + "px " + (i.R + a) + "px "
          + i.B + "px " + (i.L + a) + "px)"
    } else {
      d.vml.image.shape.style.clip = "rect(" + f.T + "px " + f.R + "px " + f.B
          + "px " + f.L + "px)"
    }
  },
  figurePercentage: function (d, c, f, a) {
    ...
    d[f] = Math.ceil(
        e ? ((c[b ? "W" : "H"] * d[f]) - (c[b ? "w" : "h"] * d[f])) : parseInt(
            a, 10));
    if (d[f] % 2 === 0) {
      d[f]++
    }
...
window.onload = function () {
  DD_belatedPNG.fix(
      "div, ul, img, em, li, input, p, span, dl, dt, dd, a, h1, h2, h3, strong,a:hover");
}