...
function setCM(b, f) {
  ...
  var c = navigator.userAgent.toUpperCase().indexOf("MSIE") == -1 ? false
      : true;
...
function LoginCheck(a, b) {
  ...
  $.post("/ajax.ashx?action=logincheck&t=" + Math.random(),
      {username: a, password: b}, function (c) {
        if (gav(c, "state") == "1") {
...
$(function () {
  ...
  function showbg(b) {
    ...
    $("#flashs .btn span.cur").css("opacity", 0.4).removeClass("cur");
  ...
  btn += "</div>";
  $("#flashs").append(btn);
  $("#flashs .btn span").css("opacity", 0.4).mouseenter(function () {
...
/*window.onerror=function(){return true};*/
function xuanze01() {
  var a = document.getElementById("seachkeywords01").value;
  if (a == "请输入产品名称") {
    $a("请输入产品名称");
    return;
  }
...
$(function () {
  ...
  $("#seachkeywords01").click(function () {
    $(this).val("").css({color: "#000"})
  }).blur(function () {
    if ($.trim($(this).val()) == "") {
      $(this).val("请输入产品名称").css({color: "#666"})
    }
  })
});
...
function IndexsendLeaveword(src) {
  ...
  if (reg.test(verContent)) {
    err += "<p>QQ不可为空</p>";
  } else if (!regcontent.test(verContent)) {
    err += "<p>QQ不正确</p>";
  }
